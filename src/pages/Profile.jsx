...
export default function Profile({
    ...
    const [blockingTotalPages, setBlockingTotalPages] = useState(1);
    ...
    const totalBlockingPages = Math.max(1, blockingTotalPages);
    const paginatedBlockedUsers = blockedUsers;
    ...
    const handleFetchProfile = useCallback(async (currentUser) => { // Hàm lấy thông tin profile của người dùng đang đăng nhập
        try {
            ...
            const response = await profileApi.getMyProfileInfo();
            const nextProfile = response.data?.result || response.data || getMockProfileByAuthUser(currentUser);
    ...
    const handleUpdateProfile = useCallback(async () => { // Hàm cập nhật thông tin account
        try {
            ...
            } else if (profileForm.avatar?.startsWith("/defaultAvatar/")) {
                formData.append("avatarPreset", profileForm.avatar);
            }

            const response = await profileApi.updateMyProfile(formData);
            const updatedProfile = response.data?.result || response.data;
    ...
    const handleChangePassword = useCallback(async () => { // Hàm đổi mk
        try {
            await profileApi.requestChangePassword({
    ...
    const handleVerifyChangePasswordOtp = useCallback(async () => { // Hàm xác nhận otp đổi mk
        try {
            ...
            await profileApi.confirmChangePassword({ otp });
    ...
    const handleToggleDisableAccount = useCallback(async () => { // Hàm bật/tắt vô hiệu hóa
        try {
            ...
            const response = nextDisabledStatus
                ? await profileApi.disableMyAccount()
                : await profileApi.enableMyAccount();
            const result = response.data?.result || response.data;
            const updatedProfile = {
                ...profileData,
                ...result,
                daVoHieuHoa: result?.daVoHieuHoa ?? nextDisabledStatus,
            };
    ...
    const handleFetchBlockedUsers = useCallback(async () => {  // Hàm lấy ds mấy đứa bị chặn (thành nhớ là 'bị user chặn' chơ kph là 'chặn th user' mô nghe)
        try {
            ...
            const response = await profileApi.getBlockedAccounts({
                keyword: blockedKeyword.trim(),
                page: Math.max(Number(blockingPage || 1) - 1, 0),
                size: BLOCKING_PAGE_SIZE,
            });
            const pageData = response.data?.result || response.data;
            const nextBlockedUsers = (pageData?.content || []).map((account) => ({
                ...account,
                blockRelationshipID: `blocked-${account.accountID}`,
                isBlocked: true,
            }));

            setBlockedUsers(nextBlockedUsers);
            setBlockingTotalPages(Math.max(1, Number(pageData?.totalPages || 1)));
        } catch (error) {
            const errorData = error.response?.data;
            const result = handleError(errorData);
    ...
    }, [addHelperError, blockedKeyword, blockingPage, handleError]);
    ...
    const handleToggleBlockUser = useCallback(
        async (targetAccount) => {
            try {
                ...
                const response = nextBlockedStatus
                    ? await profileApi.blockAccount(targetAccountID)
                    : await profileApi.unblockAccount(targetAccountID);
                const result = response.data?.result || response.data;
                const finalBlockedStatus =
                    result?.isBlocked ??
                    result?.daChan ??
                    result?.biChan ??
                    nextBlockedStatus;
                ...
                setBlockedUsers((prev) => {
                    if (!finalBlockedStatus) {
                        return prev.filter(
                            (account) => getAccountID(account) !== getAccountID(targetAccount)
                        );
                    }
    ...
    const handleBlockedKeywordChange = (event) => {
        setBlockedKeyword(event.target.value);
        setBlockingPage(1);
    };
        ...
        return (
            <div className="mx-auto max-w-3xl rounded-4xl bg-main-bg p-4 shadow-sm sm:p-6">
                <Input
                    ...
                    className="mb-5"
                    onChange={handleBlockedKeywordChange}
                />
                    ...
                    <>
                        {paginatedBlockedUsers.map((account) => (
                            <PostAuthorCard
                                ...
                                onNavigateAuthor={() => navigate(`/user/${account.accountID || account.id || account.userID || account.username}`)}
                                sideButtonConfig={{
