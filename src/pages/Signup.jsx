...
// THÊM ĐOẠN NÀY: Tạo Ref để lưu giá trị mới nhất
    ...
    const handleSendVerifyCode = async (e) => {
        try {
            await axios.post(
            "http://localhost:8080/account/send-verify-otp",
            null,
            {
                params: {
    ...
    const handleVerifyAccount = async (e) => {
        try {
            await axios.post(
                "http://localhost:8080/account/verify-and-register",
                {
                    email: formData.email,
    ...
    const handleFinalSignup = async (e) => {
        ...
        if (isAllValid) {
            try {
                await axios.post(
                    "http://localhost:8080/account/request-otp",
                    {
                        email: formData.email,
    ...
    const handleNextStep = async (e) => {
        ...
        switch (step) {
            case 1: //Kiểm tra email có ok không rồi xem có trùng CSDL không?
                if (await handleVerifyEmailSignUp()) {
                    nextStep();
                }
                break;
            default:
