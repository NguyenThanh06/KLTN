...
const PostGrid = ({ 
  ...
  onEditPost,
   ...
   useEffect(() => {
    /*
     * Infinite scroll chỉ nối thêm card xuống dưới.
     * Không chạy animation đổi trang, tránh làm toàn bộ grid nhấp nháy
     * mỗi khi người dùng cuộn để tải thêm.
     */
                ...
                {columnWrapper.map((columnPosts, colIndex) => (
                    ...
                    >
                        {columnPosts.map((post, postIndex) => {
                            const postKey = post.postId ?? post.postID ?? `${colIndex}-${postIndex}`;

                            return (
                            <PostThumbnailCard 
                            key={`${postKey}-${colIndex}-${postIndex}`} 
                            post={post} 
                            isUnder18={isUnder18}
