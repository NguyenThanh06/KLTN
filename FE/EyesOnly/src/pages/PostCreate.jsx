...
export default function PostCreate({
    ...
    /*
     * Lưu preview video riêng cho từng ảnh.
     *
     * Key: item.id của file
     * Value: blob URL video preview do BE trả về
     */
    const [previewVideoUrlByFileId, setPreviewVideoUrlByFileId] = useState({});

    /*
     * Lưu setting đã dùng để tạo preview cho từng ảnh.
     *
     * Mục đích:
     * - Nếu người dùng đổi setting, preview cũ sẽ không bị hiểu nhầm
     *   là kết quả của setting mới.
     */
    const [previewSettingsKeyByFileId, setPreviewSettingsKeyByFileId] = useState({});

    const [isPreviewLoading, setIsPreviewLoading] = useState(false);

    /*
     * Ref dùng để cleanup blob URL khi rời trang.
     */
    const previewVideoUrlByFileIdRef = useRef({});
    ...
    ///----------------- cleanup khi xóa file khỏi danh sách -----------------------
    useEffect(() => {
        const currentFileIds = new Set(
            files.map((item) => item.id)
        );

        setPreviewVideoUrlByFileId((prev) => {
            let hasChanged = false;
            const next = {};

            Object.entries(prev).forEach(([fileId, videoUrl]) => {
                if (currentFileIds.has(fileId)) {
                    next[fileId] = videoUrl;
                    return;
                }
        ...
        setPreviewSettingsKeyByFileId((prev) => {
            let hasChanged = false;
            const next = {};

            Object.entries(prev).forEach(([fileId, settingsKey]) => {
                if (currentFileIds.has(fileId)) {
                    next[fileId] = settingsKey;
                    return;
                }
    ...
    ///----------------- cleanup URL preview video khi rời trang -----------------------
    useEffect(() => {
        previewVideoUrlByFileIdRef.current =
            previewVideoUrlByFileId;
    }, [previewVideoUrlByFileId]);

    useEffect(() => {
        return () => {
            Object.values(previewVideoUrlByFileIdRef.current)
                .forEach((videoUrl) => {
                    URL.revokeObjectURL(videoUrl);
                });
        };
    ...
    const handleRefreshPreview = async () => {
        ...
        try {
            ...
            const normalizedSettings =
                normalizeProtectionSettings(currentProtectionSettings);
            ...
            formData.append("settings", JSON.stringify(normalizedSettings));

            const response = await postApi.previewProtection(formData);
            ...
            /*
             * Lưu preview riêng cho ảnh hiện tại.
             * Nếu ảnh này đã có preview cũ, thu hồi URL cũ trước.
             */
            setPreviewVideoUrlByFileId((prev) => {
                const oldUrl = prev[selectedFile.id];
                ...
                return {
                    ...prev,
                    [selectedFile.id]: nextVideoUrl,
                };
            });

            setPreviewSettingsKeyByFileId((prev) => ({
                ...prev,
                [selectedFile.id]: settingsKey,
            }));
    ...
    const currentProtectionSettingsKey = useMemo(() => {
        return JSON.stringify(
            normalizeProtectionSettings(currentProtectionSettings)
        );
    }, [currentProtectionSettings]);
    ...
    const currentPreviewVideoUrl = useMemo(() => {
        ...
        const cachedVideoUrl =
            previewVideoUrlByFileId[selectedFile.id];

        const cachedSettingsKey =
            previewSettingsKeyByFileId[selectedFile.id];

        /*
         * Chỉ dùng lại preview nếu:
         * - đúng ảnh đang chọn;
         * - setting hiện tại vẫn giống setting lúc tạo preview.
         *
         * Nếu setting đã đổi, preview cũ không còn đúng nữa,
         * nên quay về ảnh gốc và chờ người dùng bấm xem trước lại.
         */
        if (
            cachedVideoUrl &&
            cachedSettingsKey === currentProtectionSettingsKey
        ) {
        ...
        selectedFile,
        previewVideoUrlByFileId,
        previewSettingsKeyByFileId,
        currentProtectionSettingsKey,
    ]);
