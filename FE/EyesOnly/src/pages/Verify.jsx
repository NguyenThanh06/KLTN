...
const normalizePostForVerify = (rawPost) => {
    if (!rawPost) return null;

    const normalizedFiles = (rawPost.lstKTEOFile || rawPost.imageUrls || []).map((file, index) => {
        ...
        return {
            ...
            link: buildPostMediaUrl(file?.link || file?.url || file?.imageUrl),
        };
    });
...
export default function Verify({
    ...
    useEffect(() => {
        return () => {
            uploadedFilesRef.current.forEach((item) => URL.revokeObjectURL(item.url));
        };
    }, []);
    ...
    useEffect(() => {
        ...
        const fetchPostFromQuery = async () => {
            try {
                setIsFetchingPost(true);
                const response = await postApi.getPostDetail(extractPostID(postIDFromQuery));
                const foundPost = normalizePostForVerify(response.data?.result || response.data);
        ...
        fetchPostFromQuery();
    }, [addHelperError, handleError, postIDFromQuery]);
    ...
    /*
        ...
        2. Khi xác thực, FE gọi POST /verify/file/{fileID}
           với multipart field image.
    ...
    */
    const fetchPostForVerify = async (postID) => { // Hàm lấy post theo id
        ...
        return normalizePostForVerify(response.data?.result || response.data);
    };

    const submitVerifyRequest = async ({ kteoFileID, uploadedImage }) => { // Hàm verify
        const response = await postApi.verifyOriginalImage(kteoFileID, uploadedImage);
        const result = response.data?.result || response.data || {};

        return {
            isVerified: Boolean(result.verified ?? result.isVerified),
            verifyID: result.verifyID ?? result.verifyId ?? "",
        };
    };
