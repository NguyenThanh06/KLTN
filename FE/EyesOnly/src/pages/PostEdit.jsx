...
const mapPostToForm = (post) => {
    return {
        ...
        visibility: String(post?.hanCheHienThi ?? "0"),
...
export default function PostEdit({
    ...
    const userProfilePath = `/user/${currentUserID ||currentUserUsername}`;

    const deleteConfirmInputRef = useRef("");
    const handledHiddenByAdminPostRef = useRef("");
    ...
    const isHiddenByAdminReview = Number(post?.hanCheHienThi) === 99;
    ...
    const handleUpdatePost = async (payload) => { //Chỉnh sửa post
        ...
        const results = isLockedByReviewedReport
            ? [true]
            : await Promise.all([
    ...
    useEffect(() => { 
        let isMounted = true;
        handledHiddenByAdminPostRef.current = "";
        ...
        const preparePost = async () => { // Lấy thông tin post ngay mới vô
            try {
                ...
                const fetchedPost = response.data?.result || response.data;
    ...
    useEffect(() => {
        ...
        if (Number(post.hanCheHienThi) === 99) {
            if (handledHiddenByAdminPostRef.current === String(postID)) return;

            handledHiddenByAdminPostRef.current = String(postID);
    ...
    const validateBeforeSave = () => {
        ...
        if (!isLockedByReviewedReport && postForm.tags.length === 0) {
            nextErrors.tags = "empty";
        }

        if (!isLockedByReviewedReport && !postForm.visibility) {
                ...
                <div className="grid grid-cols-1 gap-8 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
                    ...
                    >
                        ...
                        <RadioGroupField
                            ...
                            options={[
                                { label: I18N_KEYS.POST_EDIT.COMMON.postEdit_formRadioLabel_hanCheHienThi_all, value: "0" },
                                { label: I18N_KEYS.POST_EDIT.COMMON.postEdit_formRadioLabel_hanCheHienThi_r18, value: "1" },
                                { label: I18N_KEYS.POST_EDIT.COMMON.postEdit_formRadioLabel_hanCheHienThi_r18g, value: "2" },
                            ]}
                        />
                        ...
                        <div className="flex flex-col-reverse gap-3 pt-2 sm:flex-row sm:justify-end">
                            ...
                            >
                                {t(isSavingPost
                                    ? I18N_KEYS.POST_EDIT.COMMON.postEdit_formButton_loading
                                    : I18N_KEYS.POST_EDIT.COMMON.postEdit_formButton_save
                                )}
                            </Button>
                        </div>
                    </form>
