...
const getAccountID = (account) => {
    if (!account) return "";
    return String(account.accountID || account.accountId || account.userID || account.id || "");
};
                    ...
                    {!isLoading && items.length === 0 ? (
                        ...
                        <div className="space-y-3">
                            {items.map((item) => {
                                ...
                                const inferredFollowing = isCurrentAccount && type === "following";
                                const isFollowingItem = isAuthenticated
                                    ? Boolean(item.daTheoDoi ?? inferredFollowing)
                                    : false;

                                return (
                                    <PostAuthorCard
                                        key={item.relationshipID || itemAccountID}
                                        author={item}
